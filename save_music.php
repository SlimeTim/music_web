...
if(!isset($resp['status'])){
    if(empty($id)){
        $sql = "INSERT INTO `music` set {$data}";
    }else{
        $sql = "UPDATE `music` set {$data} where id = '{$id}'";
    }
    $save = $conn->query($sql);
    if($save){
        ...
        if(!empty($_FILES['img']['tmp_name'])){
            ...
            if(strpos($type,"image/") > -1){
                ...
                if($move){
                    $conn->query("UPDATE `music` set image_path = CONCAT('./images/{$filename}?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$mid}'");
                }else{
